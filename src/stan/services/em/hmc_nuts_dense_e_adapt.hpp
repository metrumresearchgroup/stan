...
int hmc_nuts_dense_e_adapt(Model& model, const stan::io::var_context& init,
  ...
  model.n_sample = 0;
  size_t n_se = 100;
  model.mu_all = Eigen::VectorXd::Zero(model.n_mu);
  model.sigma_all = Eigen::MatrixXd::Zero(model.cov_rows, model.cov_cols);
...
int hmc_nuts_dense_e_adapt(
  ...
  //                    &diagnostic_writer, &pop](const tbb::blocked_range<size_t>& r) {
  //                     for (size_t i = r.begin(); i != r.end(); ++i) {
  //                       em::run_em_sampler(pop,
